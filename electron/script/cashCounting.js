...
function updateTotals() {
  ...
  if (Math.abs(difference) < 0.01) {
    messageElement.innerHTML = `<div class="success"><i class="fas fa-check-circle"></i> Perfect match! Amounts are equal</div>`;
    isValid = true;
  } else if (difference > 0) {
    messageElement.innerHTML = `<div class="warning"><i class="fas fa-info-circle"></i> Additional ₹${absDifference.toFixed(2)} needed to match deposit</div>`;
  } else {
    messageElement.innerHTML = `<div class="error"><i class="fas fa-exclamation-triangle"></i> ₹${absDifference.toFixed(2)} over the required amount</div>`;
  }
